...
var LOCALSTORAGE_PROJECT = window.localStorage[session_project];

// localStorage.graph
if (!LOCALSTORAGE_PROJECT && localStorage_txt_check == 'false') {  // AND NO localStorage.txt
...
function load_localStorage($project=false){
	if (LOCALSTORAGE_PROJECT !== "") {
		var elems = JSON.parse(LOCALSTORAGE_PROJECT);
		if (elems.length) {
			cy.add(elems);
		...
		LOCALSTORAGE_PROJECT = JSON.stringify(allJson);
...
function load_localStorage_txt($project=false){
	// console.clear();
		...
		var url_localstorage_graph_txt = 'http://127.0.0.1/WEBSTGXNEW/assets/_storygraph/__projects/1_aufildeleau/localstorage_graph.txt';
...
testAjax();
		...
		LOCALSTORAGE_PROJECT = JSON.stringify(allJson);
			...
			// LOCALSTORAGE_PROJECT = JSON.parse(LOCALSTORAGE_PROJECT);
			return elems;
...
function testAjax() {
  $.ajax({
    url: '__projects/'+session_project+'/localstorage_graph.txt', 
    type: 'GET',
    async: false,
    cache: false,
    timeout: 30000,
	...
	//error: function(msg) {
		    ...
		    //console.log('ERROR localstorage_check_txt : '+msg);
		    // load_localStorage();
		//}
  });
}
...
function callback_ok(msg){
    ...
    // console.clear();
    console.log('saveST');
    console.log(LOCALSTORAGE_PROJECT);
	...
	$('#save_ST').click(function(event, check_arrays='true'){
		...
		// console.log(LOCALSTORAGE_PROJECT);
		  $.ajax({ // todo better in a function
			...
			async: false,
		    // data: {'nodes': LOCALSTORAGE_PROJECT , 'del_dir': check_arrays},
		    data: {'nodes': LOCALSTORAGE_PROJECT },
		    success: function(msg) {
		    	// console.log(php);
		     console.log('save to txt '+msg);
