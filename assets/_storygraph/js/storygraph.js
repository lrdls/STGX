...
$(function(){
	...
	$('#clear, #clearM').click(function(){
		if (!confirm("Are you sure to delete this graph ?")) return;
		cy.remove('*');
		localStorage.graph = "";
...
/*		if (srce.hasClass('shots'))
			return false;*/
		if (srce.hasClass('sequences') && dest.hasClass('assets'))
			return false;
		if (srce.hasClass('assets') && dest.hasClass('sequences'))
			return true;
		if (srce.hasClass('assets') && dest.hasClass('assets'))
			return true;
	...
	$('#save_json').click(function(){
		...
		console.log(localStorage.graph);
		  $.ajax({ // todo better in a function
		    type: 'POST',
		    url: '../assets/_storygraph/save_graphNodes.php', // to do better
			async: false, 
	        ...
	        error: function() {
	          ...
	          console.log('ERROR saveST1 json');
	        }
		  });
		
		  $.ajax({ // todo better in a function
		    type: 'POST',
		    url: '../_storygraph/save_graphNodes.php', // to do better
			async: false, 
		    data: {'nodes': localStorage.graph },
		    success: function(msg) {
		      console.log(msg);
	        ...
	        error: function() {
	          //alert("There was an error. Try again please!");
	          console.log('ERROR saveST2 json');
	        }
		  });	  
